...
ggplot() +
  ...
  annotate("path", x=107.84-10*cos(seq(-0.3*pi,0.3*pi,length.out=30)), size = 0.6,
           y=40-10*sin(seq(-0.3*pi,0.3*pi,length.out=30)), col="black") +
  geom_point(data = ShotsDiani, aes(x = location.x, y = location.y, fill = shot.statsbomb_xg, shape = shot.body_part.name), size = 6, alpha = 0.8) + #3
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(margin = margin(r = 10, b = 10), face="bold",size = 32.5, family="serif", colour = "black", hjust = 0.5),
        legend.direction = "horizontal",
        axis.ticks = element_blank(), aspect.ratio = c(65/100),
        plot.background = element_rect(fill = "white"), strip.text.x = element_text(size=13,family="serif")) +
  labs(title = "Kadidiatou Diani, Shot Map", subtitle = "WWC, 2023") + #4
  scale_fill_gradientn(colors = shotmapxgcolors, limits = c(0, 1), name = "xG Value", 
                       breaks = c(0, 0.25, 0.5, 0.75, 1), labels = c("0", "0.25", "0.5", "0.75", "1")) + #5
  scale_shape_manual(values = shape_mapping) + #6
  guides(fill = guide_colourbar(title.position = "top"),  shape = guide_legend(override.aes = list(size = 7, fill = "black"))) + #7 
