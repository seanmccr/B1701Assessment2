...
ggplot(AtletiPDFiltStat, aes(x = reorder(Player, -GA))) +
  geom_bar(aes(y = Gls + Ast, fill = factor("Goals")), stat = "identity", position = "stack") +
  geom_bar(aes(y = Ast, fill = factor("Assists")), stat = "identity", position = "stack") +
  scale_y_continuous(breaks = seq(0, max(AtletiPDFiltStat$GA), by = 1)) +
  labs(x = "Player", y = "Number of Goals & Assists", title = "Atletico Madrid Women Strikers: Goals & Assists, 2022/23 Season") +
...
ggplot(AtletiPDFiltStat, aes(x = npxGxAG90, y = npGA90, label = Player)) +
  geom_text(check_overlap = FALSE, hjust = 0.5, vjust = -0.5) +
  geom_point() +
  geom_smooth(method = lm, color = "red") +
  labs(x = "Non-penalty Expected Goals + Assists per 90 minutes (npxGxAG90)", 
       y = "Non-penalty Goals + Assists per 90 minutes (npGA90)", 
       title = "Player Comparison of npGA90 vs npxGxAG90") +
...
ggplot(AtletiPDFiltStat, aes(x = npxGxAG, y = npGA, label = Player)) +
  geom_text(check_overlap = FALSE, hjust = 0.5, vjust = -0.5) +
  geom_point() +
  geom_smooth(method = lm, color = "red") +
  labs(x = "Non-penalty Expected Goals + Assists (npxGxAG)", 
       y = "Non-penalty Goals + Assists (npGA)", 
       title = "Player Comparison of npxGxAG vs npGA") +
  theme_minimal() +
  theme(legend.position = "right")

# Code comparing npGA to 90 minutes played
ggplot(AtletiPDFiltStat, aes(x = `90s`, y = npGA, label = Player)) +
  geom_text(check_overlap = FALSE, hjust = 0.5, vjust = -0.5) +
  geom_point() +
  geom_smooth(method = lm, color = "red") +
...
ggplot(AtletiPDFiltStat, aes(x = PrgR, y = PrgP, label = Player)) +
  geom_text(check_overlap = TRUE, hjust = 0.5, vjust = -0.5) +
  geom_point() +
  geom_smooth(method = lm, color = "red") +
  labs(x = "Progressive Passes - Received", 
       y = "Progressive Passes - Played", 
       title = "Filtered Player Comparison of Progressive Passes Played vs Received") +
  theme_minimal() +
  theme(legend.position = "right")
